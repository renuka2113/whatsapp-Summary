...
class WhatsAppChatLoader(BaseChatLoader):
    ...
    def __init__(self, path: str):
        ...
        # self._message_line_regex =  re.compile(r"(\d{2}/\d{2}/\d{2}), (\d{1,2}:\d{2})\s?[ap]m - ([^:]+): (.+)")
        self._message_line_regex =  re.compile(r"(\d{2}/\d{2}/\d{2}), (\d{1,2}:\d{2})(?:\s?[ap]m)? - ([^:]+): (.+)")
    ...
    def _load_single_chat_session(self, file_path: str) -> ChatSession:
        ...
        # for line in txt.split("\n"):
            ...
            for line in chat_lines:
                ...
                if result:
                    # date, time, meridian, sender, text = result.groups()
                    date, time, sender, text = result.groups()
                    # timestamp = f"{date}, {time} {meridian or ''}".strip()
